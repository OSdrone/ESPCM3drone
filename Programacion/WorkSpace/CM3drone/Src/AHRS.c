...
void Compensacion_Sensor_magnetico(tpAHRS *AHRS, q15_t VectorMagnetico[3]) {
	...
	// Rotamos
	mag_x = (q15_t) (((VectorMagnetico[0] * cos_pitch
			+ VectorMagnetico[1] * sin_roll) >> 15 * sin_pitch)
			+ ((VectorMagnetico[2] * cos_roll) >> 15 * sin_pitch)) >> 15;
	mag_y = (q15_t) (VectorMagnetico[1] * cos_roll
			- VectorMagnetico[2] * sin_roll) >> 15;
...
void Actualizar_Matriz_DCM_V2(tpAHRS *AHRS, q15_t VelocidadAngular[3]) {
	...
	Velocidad_Total[0] = VelocidadAngular[0]
			+ AHRS->AHRS_Config.Correccion_Proporcional[0]
			+ AHRS->AHRS_Config.Correccion_Integral[0];
	Velocidad_Total[1] = VelocidadAngular[1]
			+ AHRS->AHRS_Config.Correccion_Proporcional[1]
			+ AHRS->AHRS_Config.Correccion_Integral[1];
	Velocidad_Total[2] = VelocidadAngular[2]
			+ AHRS->AHRS_Config.Correccion_Proporcional[2]
			+ AHRS->AHRS_Config.Correccion_Integral[2];

	//Velocidad absoluta
	arm_sqrt_q15(
			((float32_t) ((Velocidad_Total[0] * Velocidad_Total[0]) >> 15))
					+ ((float32_t) ((Velocidad_Total[1] * Velocidad_Total[1])
							>> 15))
					+ ((float32_t) ((Velocidad_Total[2] * Velocidad_Total[2])
							>> 15)), &VelocidadAngularTotal);

	//Normalizamos vector rotacion TODO Revisar

	Vector_Rotacion[0] = (((int32_t) Velocidad_Total[0] << 15)
			/ VelocidadAngularTotal);
	Vector_Rotacion[1] = (((int32_t) Velocidad_Total[1] << 15)
			/ VelocidadAngularTotal);
	Vector_Rotacion[2] = (((int32_t) Velocidad_Total[2] << 15)
			/ VelocidadAngularTotal);

	Coseno = arm_cos_f32(
			VelocidadAngularTotal * AHRS->AHRS_Config.Periodo_Muestreo);
	Seno = arm_sin_f32(
			VelocidadAngularTotal * AHRS->AHRS_Config.Periodo_Muestreo);

	Rot_matriz[0][0] = (q15_t) 32768 * Coseno
			+ (q15_t) ((q15_t) (Vector_Rotacion[0] * Vector_Rotacion[0]) >> 15)
					* (1 - Coseno);
	Rot_matriz[0][1] = ((q15_t) (Vector_Rotacion[0] * Vector_Rotacion[1]) >> 15)
			* (1 - Coseno) - Vector_Rotacion[2] * Seno;
	Rot_matriz[0][2] = ((q15_t) (Vector_Rotacion[0] * Vector_Rotacion[2]) >> 15)
			* (1 - Coseno) + Vector_Rotacion[1] * Seno;
	Rot_matriz[1][0] = ((q15_t) (Vector_Rotacion[1] * Vector_Rotacion[0]) >> 15)
			* (1 - Coseno) + Vector_Rotacion[2] * Seno;
	Rot_matriz[1][1] = (q15_t) 32768 * Coseno
			+ ((q15_t) (Vector_Rotacion[1] * Vector_Rotacion[1]) >> 15)
					* (1 - Coseno);
	Rot_matriz[1][2] = ((q15_t) (Vector_Rotacion[1] * Vector_Rotacion[2]) >> 15)
			* (1 - Coseno) - Vector_Rotacion[0] * Seno;
	Rot_matriz[2][0] = ((q15_t) (Vector_Rotacion[2] * Vector_Rotacion[0]) >> 15)
			* (1 - Coseno) - Vector_Rotacion[1] * Seno;
	Rot_matriz[2][1] = ((q15_t) (Vector_Rotacion[2] * Vector_Rotacion[1]) >> 15)
			* (1 - Coseno) + Vector_Rotacion[0] * Seno;
	Rot_matriz[2][2] = (q15_t) 32768 * Coseno
			+ ((q15_t) (Vector_Rotacion[2] * Vector_Rotacion[2]))
					* (1 - Coseno);

	arm_mat_mult_q15(&AHRS->AHRS_Orientacion.DCM, &Rotacion, &Aux, NULL);
	arm_copy_q15((q15_t *) Aux_matriz,
			(q15_t *) &AHRS->AHRS_Orientacion.DCM_matriz, 9);
}
...
void Normalizar_DCM(tpAHRS *AHRS) {
	...
	arm_dot_prod_q15(&AHRS->AHRS_Orientacion.DCM_matriz[0][0],
			&AHRS->AHRS_Orientacion.DCM_matriz[1][0], 3, &error);
	error = error >> 1;
	error = -error;

	arm_scale_q15(&AHRS->AHRS_Orientacion.DCM_matriz[1][0], error, 15,
			Vector_Aux, 3);
	arm_add_q15(&AHRS->AHRS_Orientacion.DCM_matriz[0][0], Vector_Aux,
			&Matriz_Ortogonal[0][0], 3); //Vector X ortogonal

	arm_scale_q15(&AHRS->AHRS_Orientacion.DCM_matriz[0][0], error, 15,
			Vector_Aux, 3);
	arm_add_q15(&AHRS->AHRS_Orientacion.DCM_matriz[1][0], Vector_Aux,
			&Matriz_Ortogonal[1][0], 3); //Vector Y ortogonal

	//Producto Cruz
	Matriz_Ortogonal[2][0] = ((q15_t) (Matriz_Ortogonal[0][1]
			* Matriz_Ortogonal[1][2]) >> 15)
			- ((q15_t) (Matriz_Ortogonal[0][2] * Matriz_Ortogonal[1][1]) >> 15);
	Matriz_Ortogonal[2][1] = ((q15_t) (Matriz_Ortogonal[0][2]
			* Matriz_Ortogonal[1][0]) >> 15)
			- ((q15_t) (Matriz_Ortogonal[0][0] * Matriz_Ortogonal[1][2]) >> 15);
	Matriz_Ortogonal[2][2] = ((q15_t) (Matriz_Ortogonal[0][0]
			* Matriz_Ortogonal[1][1]) >> 15)
			- ((q15_t) (Matriz_Ortogonal[0][1] * Matriz_Ortogonal[1][0]) >> 15);

	arm_dot_prod_q15(&Matriz_Ortogonal[0][0], &Matriz_Ortogonal[0][0], 3,
			&error);
	error = (q15_t) ((error >> 1) * (3 - error) >> 15);
	arm_scale_q15(&Matriz_Ortogonal[0][0], error, 15,
			&AHRS->AHRS_Orientacion.DCM_matriz[0][0], 3);

	arm_dot_prod_q15(&Matriz_Ortogonal[1][0], &Matriz_Ortogonal[1][0], 3,
			&error);
	error = (q15_t) ((error >> 1) * (3 - error) >> 15);
	arm_scale_q15(&Matriz_Ortogonal[1][0], error, 15,
			&AHRS->AHRS_Orientacion.DCM_matriz[1][0], 3);

	arm_dot_prod_q15(&Matriz_Ortogonal[2][0], &Matriz_Ortogonal[2][0], 3,
			&error);
	error = (q15_t) ((error >> 1) * (3 - error) >> 15);
	arm_scale_q15(&Matriz_Ortogonal[2][0], error, 15,
			&AHRS->AHRS_Orientacion.DCM_matriz[2][0], 3);
}
...
void Correccion_deriva(tpAHRS *AHRS, q15_t AceleracionLineal[3]) {
	...
	//faltaria filtrar???
	//Producto cruz
	error[0] = ((q15_t) (AceleracionLineal[1]
			* AHRS->AHRS_Orientacion.DCM_matriz[2][2]) >> 15)
			- ((q15_t) (AceleracionLineal[2]
					* AHRS->AHRS_Orientacion.DCM_matriz[2][1]) >> 15);
	error[1] = ((q15_t) (AceleracionLineal[2]
			* AHRS->AHRS_Orientacion.DCM_matriz[2][0]) >> 15)
			- ((q15_t) (AceleracionLineal[0]
					* AHRS->AHRS_Orientacion.DCM_matriz[2][2]) >> 15);
	error[2] = ((q15_t) (AceleracionLineal[0]
			* AHRS->AHRS_Orientacion.DCM_matriz[2][1]) >> 15)
			- ((q15_t) (AceleracionLineal[1]
					* AHRS->AHRS_Orientacion.DCM_matriz[2][0]) >> 15);

	arm_scale_q15(error, AHRS->AHRS_Config.Kp_Roll_Pitch, 15,
			AHRS->AHRS_Config.Correccion_Proporcional, 3);
	arm_scale_q15(error, AHRS->AHRS_Config.Ki_Roll_Pitch, 15, Aux, 3);
	arm_add_q15(Aux, AHRS->AHRS_Config.Correccion_Integral,
			AHRS->AHRS_Config.Correccion_Integral, 3);

	//YAW
	arm_scale_q15(&AHRS->AHRS_Orientacion.DCM_matriz[2][0],
			(q15_t) (AHRS->AHRS_Orientacion.DCM_matriz[0][0]
					* arm_sin_q15(AHRS->AHRS_Orientacion.OrientacionMagnetica))
					>> 15
							- (q15_t) (AHRS->AHRS_Orientacion.DCM_matriz[1][0]
									* arm_cos_q15(
											AHRS->AHRS_Orientacion.OrientacionMagnetica))
					>> 15, 15, error, 3);

	arm_scale_q15(error, AHRS->AHRS_Config.Kp_Yaw, 15, Aux, 3);
	arm_add_q15(Aux, AHRS->AHRS_Config.Correccion_Proporcional,
			AHRS->AHRS_Config.Correccion_Proporcional, 3);
...
void Angulos_Euler(tpAHRS *AHRS) {
	AHRS->AHRS_Orientacion.Pitch = 32767
			* (-asin(AHRS->AHRS_Orientacion.DCM_matriz[2][0])) / PI;
	AHRS->AHRS_Orientacion.Roll = 32767
			* (atan2(AHRS->AHRS_Orientacion.DCM_matriz[2][1],
					AHRS->AHRS_Orientacion.DCM_matriz[2][2])) / PI;
	AHRS->AHRS_Orientacion.Yaw = 32767
			* (atan2(AHRS->AHRS_Orientacion.DCM_matriz[1][0],
					AHRS->AHRS_Orientacion.DCM_matriz[0][0])) / PI;
}
